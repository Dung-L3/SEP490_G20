...
// === helper: ép kiểu an toàn số nguyên dương (<=0 => null)
const toPosIntOrNull = (v: any) => {
  const n = Number(v);
  return Number.isInteger(n) && n > 0 ? n : null;
};

// === helper: ép kiểu an toàn số nguyên dương, có default
const toPosInt = (v: any, def = 1) => {
  const n = Number(v);
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : def;
};

// === Chuẩn hoá 1 item lấy từ localStorage để đảm bảo hỗ trợ combo & loại 0
function normalizeSelectionItem(raw: any): TakeawayItem | null {
  ...
  // Nếu có comboId hợp lệ thì combo; nếu có dishId hợp lệ thì dish; else giữ nguyên kind
...
const TakeawayOrderPublic: React.FC = () => {
  const [customerName, setCustomerName] = useState(''); // FREE: không bắt buộc (tuỳ rule)
  ...
  // Đọc món từ localStorage do /menu lưu + CHUẨN HOÁ (support combo & loại id=0)
  useEffect(() => {
    ...
    try {
      ...
      const normalized = Array.isArray(arr)
        ...
        : [];
      // ❗ Ghi đè lại localStorage sau khi normalize để “tẩy” dữ liệu 0
  ...
  const updateItemField = (idx: number, patch: Partial<TakeawayItem>) => {
    setItems((prev) => {
      const next = [...prev];
      const merged: TakeawayItem = { ...next[idx], ...patch };
  ...
  const validateForm = (): string[] => {
    ...
    // Tên khách: tuỳ rule của bạn; đang set là bắt buộc (giữ nguyên theo lần trước)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Map items -> payload: chỉ nhận id > 0
    ...
    if (mapped.length === 0) {
      setErrors(['Các món đã chọn thiếu mã món/combo. Vui lòng chọn lại từ /menu (cần có dishId/comboId > 0).']);
      return;
    }
    ...
    try {
      ...
      // Lưu info nhẹ cho lần sau
      localStorage.setItem(
        INFO_KEY,
        JSON.stringify({ customerName: trimmedName, phone: trimmedPhone })
      );
      ...
      sessionStorage.setItem(
        'chef_force_reload',
        JSON.stringify({ orderId: (resp as any)?.orderId, ts: Date.now() })
      );
            ...
            <div className="bg-gray-600 rounded-lg border border-gray-500">
              <div className="flex items-center justify-between p-3 border-b border-gray-500">
                ...
                <div className="flex gap-2">
                  <Link
                    to={`/menu?mode=takeaway&return=${returnTo}`} // return để quay lại sau khi chọn
